...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_fsharp", "queries")
            self.copy_tree("queries", dest)
        if path.isdir(path.join("fsharp_signature", "queries")):
            dest = path.join(
                self.build_lib, "tree_sitter_fsharp", "queries", "signature"
            )
            self.copy_tree(path.join("fsharp_signature", "queries"), dest)
        super().run()
...
class EggInfo(egg_info):
    def find_sources(self):
        ...
        self.filelist.include("fsharp_signature/src/tree_sitter/*.h")
...
setup(
    ...
    ext_modules=[
        Extension(
            ...
            include_dirs=["fsharp/src", "fsharp_signature/src"],
